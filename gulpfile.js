...
gulp.task('webserver', function () {
	return gulp.src(['build', 'client'])
			.pipe(webserver({
				...
				proxies: [
					{
						source: '/menu', target: 'http://localhost:8080/menu'
					},
					{
						source: '/order', target: 'http://localhost:8080/order'
					}
				]
			}));
});
...
gulp.task('build:dist', ['clean'], function (cb) {
	runSequence(['webserver', 'jshint', 'build', 'copy', 'copy:assets', 'images', 'test:unit'], 'html', cb);
});

//build files for development
gulp.task('build', ['clean'], function (cb) {
	runSequence(['sass', 'templates', 'webserver'], cb);
});
